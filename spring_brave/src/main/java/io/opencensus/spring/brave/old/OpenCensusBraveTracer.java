 ...
 */
public class OpenCensusBraveTracer implements Tracer {
  ...
  public OpenCensusBraveTracer(Sampler defaultSampler, Random random, SpanNamer spanNamer,
  ...
  public Span detach(Span span) {
    ...
    Span cur = OpenCensusBraveSpanContextHolder.getCurrentSpan();
    if (cur == null) {
    ...
    if (!span.equals(cur)) {
      ...
      OpenCensusBraveSpanContextHolder.removeCurrentSpan();
    }
  ...
  public Span close(Span span) {
    ...
    if (!span.equals(cur)) {
      ...
      OpenCensusBraveSpanContextHolder.close(new OpenCensusBraveSpanContextHolder.SpanFunction() {
        @Override public void apply(Span span) {
          OpenCensusBraveTracer.this.spanLogger.logStoppedSpan(savedSpan, span);
        }
      });
    }
    return savedSpan;
  ...
  Span createChild(Span parent, String name) {
    ...
    if (parent == null) {
      ...
      if (!isTracing()) {
        OpenCensusBraveSpanContextHolder.push(parent, true);
      }
      Span span = Span.builder().name(shortenedName)
  ...
  public Span continueSpan(Span span) {
    ...
    Span newSpan = createContinuedSpan(span, OpenCensusBraveSpanContextHolder.getCurrentSpan());
    OpenCensusBraveSpanContextHolder.setCurrentSpan(newSpan);
    return newSpan;
  }
  ...
  public Span getCurrentSpan() {
    return OpenCensusBraveSpanContextHolder.getCurrentSpan();
  }

  @Override
  public boolean isTracing() {
    return OpenCensusBraveSpanContextHolder.isTracing();
  }
